...
function Operations(elem) {
  return [...(elem.operations || []), ...elem.getInheritedOperations()].map(
    Operation,
  );
}
...
function Entity(elem) {
  return {
    ...
    fields: [...Fields(elem), ...Operations(elem), ...Relations(elem)],
  };
}
...
var entities = Repository.findAll(
  i =>
    i.stereotype &&
    (i.stereotype.name == 'node' || i.stereotype.name == 'entity'),
).map(Entity);
...
var mutations = Repository.findAll(
  i => i.stereotype && i.stereotype.name == 'mutation',
);
//
var queries = Repository.findAll(
  i => i.stereotype && i.stereotype.name == 'query',
);
//
...
fs.writeFileSync(
  './entities.ts',
  raw.run(stringify(entities, { margins: true, indent: 2 }), 'schema.njs'),
);
